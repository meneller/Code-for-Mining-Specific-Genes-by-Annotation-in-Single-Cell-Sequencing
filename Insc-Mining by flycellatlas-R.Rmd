...
annotation_cells <- colnames(adata)[adata$annotation == "young germ cell"]
...
p <- ggplot(results, aes(x = logFC, y = -log10(adj.p.value), color = annotation)) +
  ...
  scale_color_manual(values = c("without_insc" = "#8998C8", "with_insc" = "red",
                                "Not significant" = "black")) +
  theme_minimal() +
  labs(title = "Volcano Plot of Differential Gene Expression",
       subtitle = "Data source: Ovary, young germ cell",#here you change the name of the annotation you use
    x = "Log2 Fold Change",
...
with_insc_top <- subset(results, annotation == "with_insc" & abs(logFC) > 1 & -log10(adj.p.value) > 2)
...
p <- ggplot(results, aes(x = logFC, y = -log10(adj.p.value), color = annotation)) +
  geom_point(alpha = 0.6, size = 2) +
  scale_color_manual(values = c("without_insc" = "#8998C8", "with_insc" = "red", "Not significant" = "black")) +
  theme_minimal() +
  labs(
...
genes_a_anotar <- c("raskol", "mtgo", "Bsg25D", "pgc", "orb", "CG12877", "CG43340",
                    "msps", "fs(1)K10", "lok", "c(3)G", "sdt", "Kank", "insc", 
                    "CG10254", "pbl")

# Filter out genes of interest that are also “with_insc” and significant
with_insc_etiquetados <- subset(results,
                                annotation == "with_insc" &
                                abs(logFC) > 1 &
                                -log10(adj.p.value) > 2 &
                                Gene %in% genes_a_anotar)

# Volcano Plot only with those genes labeled
p <- ggplot(results, aes(x = logFC, y = -log10(adj.p.value), color = annotation)) +
  geom_point(alpha = 0.6, size = 2) +
  scale_color_manual(values = c("without_insc" = "#8998C8", 
                                "with_insc" = "red", 
                                "Not significant" = "black")) +
  theme_minimal() +
  labs(
  ...
  geom_hline(yintercept = -log10(0.05), color = "grey", linetype = "dashed") +
  geom_vline(xintercept = c(-1, 1), color = "grey", linetype = "dashed") +
  geom_text_repel(data = with_insc_etiquetados, aes(label = Gene), size = 3) +
  theme(
    text = element_text(family = "Arial", size = 12),
    legend.position = "top",
    legend.title = element_blank()
